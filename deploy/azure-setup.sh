...
set -euo pipefail
export MSYS_NO_PATHCONV=1   # Git Bash on Windows would rewrite /healthz and /subscriptions/... into file paths
...
# "enabled<TAB>action" from the app's authConfig (field paths differ between CLI versions).
auth_state() {
  tsv containerapp auth show -n "$APP" -g "$RG" --query \
    "[properties.platform.enabled || platform.enabled, properties.globalValidation.unauthenticatedClientAction || globalValidation.unauthenticatedClientAction]" \
    2>/dev/null || true
}

signin_enforced() {
  local state; state=$(auth_state)
  [ "$(cut -f1 <<<"$state")" = true ] && [ "$(cut -f2 <<<"$state")" = RedirectToLoginPage ]
}
...
cmd_google() {
  ...
  az containerapp auth update -n "$APP" -g "$RG" --enabled true \
...
cmd_ci() {
  ...
  az identity federated-credential create --name gh-main --identity-name "$IDENTITY" -g "$RG" \
  ...
  n=$(tsv role assignment list --scope "$rg_id" \
