...
export const ERROR_PATTERNS: Record<ErrorPatternId, ErrorPatternInfo> = {
  ...
  // --- domain, range, composition (content maps FunctionMistakeKind → fn_*; engine writes the witness) ---
  fn_domain_forgot_denominator: {
    id: 'fn_domain_forgot_denominator',
    title: 'A denominator can never be 0',
    lesson: 'If a formula divides, the bottom is not allowed to be 0, even when everything else looks fine. Solve bottom ≠ 0 and leave those x-values out.',
    example: 'sqrt(x + 2)/(x − 3) → [−2, ∞) ✗ → [−2, 3) ∪ (3, ∞)',
  },
  fn_domain_root_strict: {
    id: 'fn_domain_root_strict',
    title: 'The square root of 0 is 0',
    lesson: 'An even root needs its inside to be at least 0, and 0 itself is allowed. A parenthesis where a bracket belongs drops a point where f is 0, which is defined.',
    example: 'sqrt(x + 2) → (−2, ∞) ✗ → [−2, ∞)',
  },
  fn_domain_root_denominator_zero: {
    id: 'fn_domain_root_denominator_zero',
    title: 'A root in a denominator must be positive',
    lesson: 'An even root in a denominator cannot be 0, because that is dividing by 0. The inside has to be strictly greater than 0, not greater than or equal.',
    example: '1/sqrt(x − 1) → [1, ∞) ✗ → (1, ∞)',
  },
  fn_domain_odd_root_restricted: {
    id: 'fn_domain_odd_root_restricted',
    title: 'Cube roots take negative numbers',
    lesson: 'The cube root of a negative number is a real number (cbrt(−8) = −2). Only even roots, like square roots, need the inside to be at least 0.',
    example: 'cbrt(x − 2) → [2, ∞) ✗ → (−∞, ∞)',
  },
  fn_domain_no_flip: {
    id: 'fn_domain_no_flip',
    title: 'Dividing by a negative flips the inequality',
    lesson: 'When you divide both sides by a negative number, the inequality sign flips. A negative in front of x inside a root is exactly that step.',
    example: 'sqrt(−2x + 6) → [3, ∞) ✗ → (−∞, 3]',
  },
  fn_domain_gave_range: {
    id: 'fn_domain_gave_range',
    title: 'That is the range, not the domain',
    lesson: 'The domain is the set of inputs x that f accepts. The range is the set of outputs. They answer different questions.',
    example: 'sqrt(−2x + 6) → [0, ∞) ✗ → (−∞, 3]',
  },
  fn_domain_denominator_nonneg: {
    id: 'fn_domain_denominator_nonneg',
    title: 'A denominator only has to be nonzero',
    lesson: 'The bottom of a fraction can be negative. It just cannot be 0. Writing bottom ≥ 0 (or > 0) throws away half the inputs.',
    example: '1/(x − 3) → [3, ∞) ✗ → (−∞, 3) ∪ (3, ∞)',
  },
  fn_range_gave_domain: {
    id: 'fn_range_gave_domain',
    title: 'That is the domain, not the range',
    lesson: 'The range is the set of outputs y = f(x). The domain is the inputs. A set of x-values is not the range.',
    example: '−2sqrt(x + 1) + 5 → [−1, ∞) ✗ → (−∞, 5]',
  },
  fn_range_reflection_ignored: {
    id: 'fn_range_reflection_ignored',
    title: 'A negative in front flips the outputs',
    lesson: 'Multiplying by a negative turns the graph upside down. A downward parabola, or a negative in front of a square root or absolute value, sends the outputs the other way.',
    example: '−2sqrt(x + 1) + 5 → [5, ∞) ✗ → (−∞, 5]',
  },
  fn_range_included_asymptote: {
    id: 'fn_range_included_asymptote',
    title: 'The asymptote value is not an output',
    lesson: 'A horizontal asymptote is a height the graph gets close to and never reaches. That y-value is left out of the range.',
    example: '3/(x − 2) + 1 → (−∞, ∞) ✗ → (−∞, 1) ∪ (1, ∞)',
  },
  fn_compose_no_parens: {
    id: 'fn_compose_no_parens',
    title: 'Put the inside function in parentheses',
    lesson: 'When you substitute, the whole of g(x) goes where x was, in parentheses. Without them, an exponent or a coefficient only reaches part of g.',
    example: 'x − 3^2 + 1 ✗ → (x − 3)^2 + 1',
  },
  fn_compose_partial_sub: {
    id: 'fn_compose_partial_sub',
    title: 'Every x in f becomes g(x)',
    lesson: 'Substitution replaces every x in f, not just the first one. If any x is left as itself, that term never saw g.',
    example: '(x + 1)^2 + 2x ✗ → (x + 1)^2 + 2(x + 1)',
  },
  fn_compose_product: {
    id: 'fn_compose_product',
    title: 'Composition is not multiplication',
    lesson: 'f(g(x)) puts g inside f. It is not f(x) times g(x).',
    example: '(x^2 + 1)(x − 3) ✗ → (x − 3)^2 + 1',
  },
  fn_compose_reversed: {
    id: 'fn_compose_reversed',
    title: 'f ∘ g means f on the outside',
    lesson: '(f ∘ g)(x) is f(g(x)): g goes first, then f. g(f(x)) is the other order.',
    example: '(x^2 + 1) − 3 ✗ → (x − 3)^2 + 1',
  },
  fn_compose_sum: {
    id: 'fn_compose_sum',
    title: 'Composition is not addition',
    lesson: 'f(g(x)) puts g inside f. Adding the two formulas is f(x) + g(x), a different function.',
    example: 'x^2 + 1 + (x − 3) ✗ → (x − 3)^2 + 1',
  },
  fn_value_product: {
    id: 'fn_value_product',
    title: '(f ∘ g)(a) is not f(a) times g(a)',
    lesson: 'A number in parentheses means plug it in, from the inside out: g(a) first, then f of that result. Multiplying f(a) by g(a) is a different calculation.',
    example: 'f(2) · g(2) = 27 ✗ → f(g(2)) = 11',
  },
  fn_value_reversed: {
    id: 'fn_value_reversed',
    title: 'Inside first: g(a), then f',
    lesson: '(f ∘ g)(a) works from the inside out. g(f(a)) is the other order, (g ∘ f)(a).',
    example: 'g(f(2)) = 80 ✗ → f(g(2)) = 11',
  },
  fn_composite_domain_simplified: {
    id: 'fn_composite_domain_simplified',
    title: 'Simplifying can hide a restriction',
    lesson: 'The domain of f ∘ g is not whatever the simplified formula allows. x has to be in the domain of g first, and then g(x) has to be in the domain of f.',
    example: 'f(x) = x^2, g(x) = sqrt(x) → (−∞, ∞) ✗ → [0, ∞)',
  },
  fn_composite_domain_inner_only: {
    id: 'fn_composite_domain_inner_only',
    title: 'g(x) still has to be allowed into f',
