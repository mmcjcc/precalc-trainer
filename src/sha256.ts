/** SHA-256 hex. Uses Web Crypto when available (HTTPS / localhost); otherwise a JS fallback
 *  so the PIN gate works on a LAN IP over plain HTTP. */
...
export async function sha256hex(message: string): Promise<string> {
  ...
  if (subtle) {
    try {
      const buf = await subtle.digest('SHA-256', new TextEncoder().encode(message))
      return hex(new Uint8Array(buf))
    } catch {
      /* insecure context (http://192.168.x.x) */
    }
  }
